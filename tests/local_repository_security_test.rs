...
fn test_prevent_directory_traversal() {
    ...
    for path_str in valid_paths {
        ...
        assert!(prevent_directory_traversal(&path).is_ok(),
            "Path '{}' should be considered safe but was rejected", path_str);
    }
    ...
    for path_str in invalid_paths {
        let path = PathBuf::from(path_str);
        assert!(prevent_directory_traversal(&path).is_err(),
            "Path '{}' contains directory traversal but was not rejected", path_str);
    }
}
...
fn test_normalize_repository_path_security() {
    ...
    assert_eq!(normalized1, path1, "Path without leading slash should remain unchanged");
    ...
    assert_eq!(normalized2, PathBuf::from("file.txt"), "Path with leading slash should have it removed");
    ...
    for path_str in invalid_paths {
        let path = PathBuf::from(path_str);
        assert!(repo.normalize_repository_path(&path).is_err(),
            "Path '{}' should fail normalization due to directory traversal", path_str);
    }
}
...
fn test_view_file_params_security() {
    ...
    assert!(repo.normalize_repository_path(&valid_params.file_path).is_ok(),
    ...
    assert!(repo.normalize_repository_path(&invalid_params.file_path).is_err(),
        "Invalid ViewFileParams should fail path normalization");

    // Ensure prevent_directory_traversal catches the invalid path
    assert!(prevent_directory_traversal(&invalid_params.file_path).is_err(),
...
fn test_comprehensive_path_security() {
    ...
    for path_str in exploit_attempts {
        let path = PathBuf::from(path_str);
        assert!(prevent_directory_traversal(&path).is_err(),
            "Security exploit path '{}' was not properly rejected", path_str);
    }
}
...
fn test_directory_traversal_in_exclude_dirs() {
    ...
    for dir in valid_dirs {
        assert!(prevent_directory_traversal(&PathBuf::from(dir)).is_ok(),
            "Valid directory path '{}' should be accepted", dir);
    }
    ...
    for dir in invalid_dirs {
        assert!(prevent_directory_traversal(&PathBuf::from(dir)).is_err(),
            "Invalid directory path '{}' should be rejected", dir);
    }
}
...
async fn test_search_code_integration() {
    ...
    assert!(result.is_err(), "search_code should reject parameters with directory traversal in exclude_dirs");
    let error_message = result.err().unwrap();
    assert!(error_message.contains("Invalid exclude_dir path"), 
        "Error message should indicate the problem is with exclude_dir path: {}", error_message);
}