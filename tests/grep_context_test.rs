...
async fn test_grep_no_context() {
    ...
    let no_context_matches: Vec<_> = result.matches.iter()
        .filter(|m| m.file_path.ends_with("no_context.txt"))
        .collect();
    
    // We should have 1 match from no_context.txt, with no context lines
    assert_eq!(no_context_matches.len(), 1, "Expected 1 match from no_context.txt");
    
    // Verify that only the matching line is included (not context lines)
    assert!(no_context_matches[0].line_content.contains("pattern"), "Match should contain the pattern");
    assert!(!no_context_matches[0].is_context, "Match should not be marked as context");
    
    // Make sure this match has no context flags
    let context_matches = result.matches.iter()
        .filter(|m| m.file_path.ends_with("no_context.txt") && m.is_context)
        .count();
    assert_eq!(context_matches, 0, "Should not have any context lines marked");
}
...
async fn test_grep_before_context() {
    ...
    // We expect one match 
    assert!(result.matches.len() >= 1, "Expected at least one match, got {}", result.matches.len());
    ...
    for (i, m) in result.matches.iter().enumerate() {
        println!("Match #{}: '{}' at line {}", i+1, m.line_content.trim(), m.line_number);
    }
    ...
    let minimal_matches: Vec<_> = result.matches.iter()
    ...
    assert_eq!(minimal_matches.len(), 2, "Expected 2 matches from minimal.txt");
    
    // The first should be the context line, the second should be the actual match
    assert!(minimal_matches[0].line_content.contains("line2"), "First match should be context line");
    assert!(minimal_matches[0].is_context, "First match should be marked as context");
    assert!(minimal_matches[1].line_content.contains("pattern"), "Second match should contain the pattern");
    assert!(!minimal_matches[1].is_context, "Second match should not be marked as context");
...
async fn test_grep_after_context() {
    ...
    let after_matches: Vec<_> = result.matches.iter()
    ...
    assert!(after_matches[0].line_content.contains("pattern"), "First match should contain the pattern");
    assert!(!after_matches[0].is_context, "First match should not be marked as context");
    assert!(after_matches[1].line_content.contains("line3"), "Second match should be context line");
    assert!(after_matches[1].is_context, "Second match should be marked as context");
}
...
async fn test_grep_before_after_context() {
    ...
    let both_matches: Vec<_> = result.matches.iter()
    ...
    let before_lines = both_matches.iter().filter(|m| m.is_context && m.line_content.contains("line2")).count();
    let match_lines = both_matches.iter().filter(|m| !m.is_context && m.line_content.contains("pattern")).count();
    let after_lines = both_matches.iter().filter(|m| m.is_context && m.line_content.contains("line4")).count();
